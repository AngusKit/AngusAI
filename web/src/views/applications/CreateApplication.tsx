...
type ModelType = 'gpt-4' | 'gpt-4-turbo' | 'gpt-3.5-turbo' | 'claude-3-opus' | 'claude-3-sonnet' | 'claude-3-haiku' | 'llama-3-70b' | 'llama-3-8b' | 'gemini-pro' | 'mistral-large';
type TemplateType = 'modern-blue' | 'minimal-gray' | 'elegant-purple' | 'warm-orange';
type IconType = 'message' | 'sparkles' | 'help' | 'bot' | 'zap' | 'database' | 'file' | 'users' | 'palette' | 'image' | 'smile' | 'star' | 'heart' | 'briefcase' | 'coffee' | 'globe' | 'bookmark' | 'cloud' | 'cpu' | 'music' | 'video' | 'camera' | 'mail' | 'phone' | 'map' | 'calendar' | 'bell' | 'settings' | 'folder' | 'archive' | 'download' | 'upload' | 'share' | 'lock' | 'unlock' | 'eye' | 'eyeoff' | 'edit' | 'trash' | 'copy' | 'link' | 'hash' | 'percent' | 'trending' | 'barchart' | 'piechart' | 'activity' | 'target' | 'flag' | 'award' | 'gift' | 'rocket' | 'layers' | 'command';
          ...
          <div key={step.number} className="flex items-center">
            <div className="flex flex-col items-center relative">
              ...
              <div className="mt-3 text-center">
                ...
                <div className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-[100px]">
                  {step.description}
                </div>
              </div>
            </div>
...
export function CreateApplication({ onBack }: { onBack: () => void }) {
  ...
  const steps = [
    { number: 1, title: t('createApp.selectType'), description: t('createApp.selectTypeDesc') },
    { number: 2, title: t('createApp.configModel'), description: t('createApp.configModelDesc') },
    { number: 3, title: t('createApp.setParams'), description: t('createApp.setParamsDesc') },
    { number: 4, title: '关联资源', description: '知识库、数据集、工作流、接口' },
    { number: 5, title: t('createApp.previewComplete'), description: t('createApp.previewCompleteDesc') },
  ];
  ...
  const getTypeSpecificDefaults = () => {
    switch (selectedType) {
      case 'chatbot':
        return {
          namePrefix: '智能客服助手',
          descriptionTemplate: '这是一个智能客服对话助手，支持多轮对话和上下文理解，能够快速理解用户需求并提供专业的解决方案。',
          welcomeTemplate: '您好！我是智能客服助手，有什么可以帮助您的吗？',
        };
      case 'text-generation':
        return {
          namePrefix: '内容创作助手',
          descriptionTemplate: '这是一个AI内容创作工具，可以自动生成高质量的文章、广告文案和社交媒体内容，帮助您快速完成内容创作。',
          welcomeTemplate: '欢迎使用内容创作助手！请告诉我您需要创作什么类型的内容。',
        };
      case 'knowledge':
        return {
          namePrefix: '知识库问答助手',
          descriptionTemplate: '这是一个基于知识库的智能问答系统，可以快速检索相关信息并提供准确的答案，支持多语言和文档管理。',
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`${type.iconBg} w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg`}>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                    ...
                    <Switch
                      checked={enableLongConversation}
                      onCheckedChange={setEnableLongConversation}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                    ...
                    <Switch
                      checked={enablePreciseAnalysis}
                      onCheckedChange={setEnablePreciseAnalysis}
                    />
                  </div>
                </div>
              </div>
                ...
                {templates.map((template) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <div className={`w-8 h-8 ${template.secondaryColor} rounded border ${template.accentColor}`}></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="p-4 space-y-3 h-64 overflow-y-auto">
                  ...
                  <div className="flex gap-2">
                    <div className={`${selectedTemplateObj?.primaryColor} w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0`}>
                      <SelectedIcon className="w-4 h-4 text-white" />
                    </div>
                    <div className={`${selectedTemplateObj?.secondaryColor} p-3 rounded-lg rounded-tl-none max-w-[80%]`}>
  ...
  const renderStep4 = () => {
    ...
    const knowledgeBases = [
      { id: 1, name: '产品文档', description: '公司所有产品的使用说明和技术文档', icon: '📘', documentCount: '24', size: '456 MB', enabled: true },
      { id: 2, name: '培训资料', description: '新员工培训及各类培训资料汇总', icon: '📚', documentCount: '13', size: '312 MB', enabled: true },
      { id: 3, name: '市场分析', description: '市场调研报告和竞品分析文档', icon: '📊', documentCount: '7', size: '703 MB', enabled: false },
      { id: 4, name: '客户案例', description: '成功案例和客户故事集', icon: '📝', documentCount: '18', size: '234 MB', enabled: true },
      { id: 5, name: '技术文档', description: 'API文档和开发指南', icon: '💻', documentCount: '31', size: '567 MB', enabled: true },
    ];

    // 模拟数据集数据
    const datasets = [
      { id: 1, name: '客户对话数据集', description: '基于客服场景的真实对话数据', icon: '📚', type: '文本', dataCount: '12.5K 条', enabled: true },
      { id: 2, name: '技术问答集', description: 'IT技术类常见问题及答案', icon: '📗', type: '表格', dataCount: '5.2K 条', enabled: true },
      { id: 3, name: '产品评价数据集', description: '电商平台产品评价及推荐相关', icon: '📙', type: '文本', dataCount: '18.7K 条', enabled: false },
      { id: 4, name: 'CRM客户数据源', description: '连接到CRM系统的客户数据', icon: '🔌', type: '数据源', dataCount: '45.6K 条', enabled: true },
      { id: 5, name: '销售数据集', description: '历史销售数据和趋势分析', icon: '📈', type: '表格', dataCount: '32.1K 条', enabled: true },
    ];

    // 模拟工作流数据
    const workflows = [
      { id: 1, name: '客户服务流程', description: '标准的客户服务工作流，包含问题分类、自动回复、人工转接', icon: '🔄', nodeCount: '8', status: '已发布' },
      { id: 2, name: '数据分析流程', description: '自动化数据收集、清洗、分析和报告生成', icon: '📊', nodeCount: '12', status: '已发布' },
      { id: 3, name: '内容审核流程', description: 'AI辅助的内容审核和过滤工作流', icon: '✅', nodeCount: '6', status: '已发布' },
      { id: 4, name: '文档处理流程', description: '文档上传、解析、分类和归档的自动化流程', icon: '📄', nodeCount: '10', status: '已发布' },
    ];

    // 模拟接口集数据
    const apiCollections = [
      { id: 1, name: 'OpenAI API', description: 'OpenAI GPT-4 和相关模型的接口集合', icon: '🤖', endpointCount: '15', source: 'OpenAPI', enabled: true },
      { id: 2, name: '支付服务 API', description: '支付宝、微信支付等第三方支付接口', icon: '💳', endpointCount: '28', source: 'Swagger', enabled: true },
      { id: 3, name: '电商平台 API', description: '商品管理、订单处理、库存系统接口', icon: '🛒', endpointCount: '45', source: 'Postman', enabled: true },
      { id: 4, name: '天气数据 API', description: '实时天气数据和预报接口', icon: '🌤️', endpointCount: '8', source: 'OpenAPI', enabled: false },
      { id: 5, name: '地图服务 API', description: '地理位置、路径规划等地图服务', icon: '🗺️', endpointCount: '22', source: 'OpenAPI', enabled: true },
    ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
              ...
              <ScrollArea className="h-[400px] pr-4">
                <div className="space-y-3">
                  {knowledgeBases.filter(kb => kb.enabled).map((kb) => (
                    <Card
                      key={kb.id}
          ...
          <div>
            <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedDataset && (
                  <Badge className="bg-green-500">已选择</Badge>
                )}
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                选择一个数据集为应用提供训练数据（单选）
              ...
              <ScrollArea className="h-[400px] pr-4">
                <div className="space-y-3">
                  {datasets.filter(ds => ds.enabled).map((ds) => (
                    <Card
                      key={ds.id}
          ...
          <div>
            <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedWorkflow && (
                  <Badge className="bg-purple-500">已选择</Badge>
                )}
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          ...
          <div>
            <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
              ...
              <ScrollArea className="h-[400px] pr-4">
                <div className="space-y-3">
                  {apiCollections.filter(api => api.enabled).map((api) => (
                    <Card
                      key={api.id}
          ...
          <div>
            <Card className="p-6 dark:bg-gray-800 dark:border-gray-700">
              ...
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                <div className={`${selectedTypeObj?.iconBg} w-16 h-16 rounded-xl flex items-center justify-center shadow-lg`}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge className={enableLongConversation 
                      ? 'bg-green-500 hover:bg-green-600' 
                      : 'bg-gray-400 hover:bg-gray-500'
                    }>
                      长对话性：{enableLongConversation ? '开启' : '关闭'}
                    </Badge>
                    <Badge className={enablePreciseAnalysis 
                      ? 'bg-green-500 hover:bg-green-600' 
                      : 'bg-gray-400 hover:bg-gray-500'
                    }>
                  ...
                  <div className="space-y-3 pr-4">
                    ...
                    {testMessages.length === 0 && (
                      <div className="flex gap-2">
                        ...
                        <div className={`${selectedTemplateObj?.secondaryColor} p-3 rounded-lg rounded-tl-none max-w-[85%]`}>
                          <p className="text-sm dark:text-white">
