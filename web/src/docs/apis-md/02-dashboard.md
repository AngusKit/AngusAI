...
**接口路径**：`GET /api/v1/dashboard/notifications`  
**接口说明**：获取系统通知和重要提醒
...
```typescript
{
  ...
  data: {
    items: [
      {
        id: number;
        type: 'system' | 'billing' | 'team' | 'application';
        title: string;
        content: string;
        isRead: boolean;
        createdDate: Date
...
**接口路径**：`PATCH /api/v1/dashboard/notifications/:id/read`  
**接口说明**：标记指定通知为已读
...
**接口路径**：`POST /api/v1/dashboard/notifications/read-all`  
**接口说明**：标记所有通知为已读
