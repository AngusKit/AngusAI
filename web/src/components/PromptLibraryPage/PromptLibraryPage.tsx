...
const TAG_COLORS = [
  { name: '蓝色', nameEn: 'Blue', value: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  { name: '紫色', nameEn: 'Purple', value: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  { name: '绿色', nameEn: 'Green', value: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' },
  { name: '橙色', nameEn: 'Orange', value: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
  { name: '红色', nameEn: 'Red', value: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
  { name: '粉色', nameEn: 'Pink', value: 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400' },
  { name: '青色', nameEn: 'Cyan', value: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' },
  { name: '黄色', nameEn: 'Yellow', value: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' },
  { name: '靛青', nameEn: 'Indigo', value: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' },
  { name: '紫罗兰', nameEn: 'Violet', value: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400' },
];
...
export function PromptLibraryPage() {
  ...
  const [categories, setCategories] = useState<Category[]>([
    { id: 'all', name: '全部', nameEn: 'All', icon: Sparkles, color: 'text-gray-700 dark:text-gray-300', isSystem: true },
    { id: 'favorites', name: '收藏', nameEn: 'Favorites', icon: Star, color: 'text-yellow-600 dark:text-yellow-500', isSystem: true },
    { id: 'coding', name: '编程开发', nameEn: 'Coding', icon: Code, color: 'text-blue-600 dark:text-blue-400', isSystem: true },
    { id: 'coding-frontend', name: '前端开发', nameEn: 'Frontend', icon: Code, color: 'text-blue-500 dark:text-blue-300', isSystem: true, parentId: 'coding' },
    { id: 'coding-backend', name: '后端开发', nameEn: 'Backend', icon: Code, color: 'text-blue-500 dark:text-blue-300', isSystem: true, parentId: 'coding' },
    { id: 'writing', name: '写作', nameEn: 'Writing', icon: BookOpen, color: 'text-green-600 dark:text-green-400', isSystem: true },
    { id: 'marketing', name: '营销', nameEn: 'Marketing', icon: TrendingUp, color: 'text-purple-600 dark:text-purple-400', isSystem: true },
    { id: 'productivity', name: '生产力', nameEn: 'Productivity', icon: MessageSquare, color: 'text-orange-600 dark:text-orange-400', isSystem: true },
    { id: 'custom-1', name: '我的自定义分类', nameEn: 'My Custom Category', icon: BookOpen, color: 'text-pink-600 dark:text-pink-400', isSystem: false },
  ]);
  ...
  const [prompts, setPrompts] = useState<Prompt[]>([
    ...
    },
    {
      ...
      category: 'productivity',
      tags: [
        { label: '自定义', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
      ],
      isFavorite: false,
      usageCount: 5,
      isSystem: false,
  ...
  const openEditDialog = (prompt: Prompt) => {
    // 检查是否为系统模板
    if (prompt.isSystem) {
      toast.error(language === 'zh-CN' ? '系统模板不可编辑，可以复制后修改' : 'System templates cannot be edited, please duplicate and modify');
      return;
    }
    ...
    if (deletingCategory) {
      ...
      if (promptsInCategory.length > 0) {
        ...
        setPrompts(prev => prev.map(p => 
          allCategoryIds.includes(p.category)
            ? { ...p, category: 'coding' } 
            : p
        ));
      }
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge variant="outline" className="text-xs">
                            {language === 'zh-CN' ? '使用' : ''} {prompt.usageCount} {language === 'zh-CN' ? '次' : 'uses'}
                          </Badge>
                        </div>
      ...
      <Dialog open={showPromptDialog} onOpenChange={setShowPromptDialog}>
        <DialogContent className="max-w-2xl dark:bg-gray-800">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingPrompt 
                ? (language === 'zh-CN' ? '修改提示词信息' : 'Edit prompt information')
                : (language === 'zh-CN' ? '创建一个新的提示词模板' : 'Create a new prompt template')}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showCategoryDialog} onOpenChange={setShowCategoryDialog}>
        <DialogContent className="dark:bg-gray-800">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="parentCategory">{language === 'zh-CN' ? '父分组（可选）' : 'Parent Category (Optional)'}</Label>
              <Select
                value={categoryForm.parentId || 'none'}
                onValueChange={value => setCategoryForm(prev => ({ ...prev, parentId: value === 'none' ? '' : value }))}
              >
                <SelectTrigger className="dark:bg-gray-900 dark:border-gray-700">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="dark:bg-gray-800 dark:border-gray-700">
                  <SelectItem value="none">{language === 'zh-CN' ? '无（作为顶层分组）' : 'None (Top-level category)'}</SelectItem>
