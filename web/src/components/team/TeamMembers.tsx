...
export function TeamMembers() {
  ...
  const stats = [
    { label: '团队成员', value: members.length, subtext: '活跃成员', icon: Users, color: 'text-blue-600' },
    { label: '待处理邀请', value: pendingInvitations.length, subtext: '等待接受', icon: Mail, color: 'text-orange-600' },
    { label: '管理员', value: members.filter(m => m.role === 'admin' || m.role === 'owner').length, subtext: '拥有管理权限', icon: Shield, color: 'text-purple-600' },
    { label: '活跃率', value: '87%', subtext: '过去7天', icon: CheckCircle, color: 'text-green-600' }
  ];
                  ...
                  <PaginationContent>
                    ...
                    <PaginationItem>
                      <PaginationNext 
                        ...
                        className={currentInvitePage === totalInvitePages ? 'pointer-events-none opacity-50' : 'cursor-pointer'}
                      >
                        下一页
                      </PaginationNext>
                    </PaginationItem>
