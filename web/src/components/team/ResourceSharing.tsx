...
export function ResourceSharing() {
  ...
  const getPermissionBadge = (permission: string) => {
    const badges = {
      view: { label: '查看', color: 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-400', icon: Eye },
      edit: { label: '编辑', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: Edit },
      manage: { label: '管理', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: Shield }
    };
  ...
  const handleShare = () => {
    ...
    if (shareType === 'specific') {
      ...
      const resourceName = availableResources[selectedResourceType as keyof typeof availableResources]
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          setTeamMembers(
                            teamMembers.map((m) =>
                              m.id === member.id ? { ...m, selected: checked as boolean } : m
                            )
                          );
                        }}
                      />
