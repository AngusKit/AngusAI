...
# ---------------------------------------------------------------------------
# Jenkins CI/CD Pipeline Script for AngusAI Project.
...
clone_repository() {
  ...
  GIT_URL="https://github.com/xcancloud/AngusAI.git"

  if [ -n "$gitCredential" ]; then
    GIT_URL="https://${gitCredential}@github.com/xcancloud/AngusAI.git"
  fi
...
deploy_service() {
  ...
  ssh "$host" "cd ${REMOTE_APP_DIR} && sh shutdown-ai.sh" || {
    echo "WARN: Failed to stop service, proceeding anyway"
  }
  ssh "$host" "cd ${REMOTE_APP_DIR} && find . -mindepth 1 -maxdepth 1 -not \( -name ${REMOTE_APP_LOGS_DIR_NAME} -o -name ${REMOTE_APP_DATA_DIR_NAME} -o -name ${REMOTE_APP_STATIC_DIR_NAME} \) -exec rm -rf {} +" || {
...
#  }
  ...
  ssh "$host" "cd ${REMOTE_APP_DIR} && sh startup-ai.sh --debug" || {
