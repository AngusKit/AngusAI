...
public interface DatasetDataRepo extends BaseRepository<DatasetData, Long> {
  ...
  /**
   * 按数据集ID批量统计 返回 List&lt;Object[]&gt; [datasetId, totalFilesOrTables, totalRecords,
   * totalRecordsSize]
   */
  @Query(
      "SELECT dd.datasetId, COUNT(dd), COALESCE(SUM(dd.dataCount), 0), COALESCE(SUM(dd.dataSize), 0) "
