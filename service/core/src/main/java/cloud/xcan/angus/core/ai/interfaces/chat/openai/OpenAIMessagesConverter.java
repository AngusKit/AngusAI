...
/**
 * OpenAI messages 格式转换器
 * 将 messages 转为 AgentRegistry 所需的单条消息输入
 */
public final class OpenAIMessagesConverter {
  ...
  /**
   * 从 messages 中提取用于 Agent 的输入消息
   * 规则：
   * 1. 若有 sessionId，仅传最后一条 user 消息（依赖服务端记忆）
   * 2. 若无 sessionId，将全量 user/assistant 历史格式化为上下文 + 最后 user 消息
   *
