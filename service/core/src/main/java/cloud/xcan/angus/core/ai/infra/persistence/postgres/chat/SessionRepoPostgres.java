...
public interface SessionRepoPostgres extends SessionRepo {
  ...
  @Override
  @Query(value =
      "SELECT EXTRACT(MONTH FROM s.created_date)::int AS mth, COUNT(*) FROM ai_chat_session s "
  ...
  @Override
  @Query(value =
      "SELECT EXTRACT(DAY FROM s.created_date)::int AS d, COUNT(*) FROM ai_chat_session s "
  ...
  @Override
  @Query(value =
      "SELECT EXTRACT(HOUR FROM s.created_date)::int AS h, COUNT(*) FROM ai_chat_session s "
