...
public interface ResourceSharingAccessLogRepo extends
   ...
   * 根据共享ID查询访问日志
   */
  Page<ResourceSharingAccessLog> findBySharingIdOrderByCreatedDateDesc(Long sharingId,
      Pageable pageable);

  /**
   * 根据共享ID和用户ID查询访问日志
   */
  List<ResourceSharingAccessLog> findBySharingIdAndUserIdOrderByCreatedDateDesc(Long sharingId,
      Long userId);
   ...
   */
  @Query(
      "SELECT DATE(l.createdDate) as date, COUNT(l) as count, COUNT(DISTINCT l.userId) as users " +
